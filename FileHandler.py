...
def load_frames(uid: str, path: str):
    frames = []
    for entry in os.listdir(path):
        if uid in entry:
            frames.append(os.path.join(path, entry))
    frames.sort(key=lambda i: int((os.path.splitext(os.path.basename(i))[0]).split('_')[-1]))
...
def match_tv_episode(filename):
    match = re.search('''S\d{1,2}([-+]?E\d{1,2})+''', filename, re.IGNORECASE)
    if match:
